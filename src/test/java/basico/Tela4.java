...
public class Tela4 {
    ...
    public void preencherTela4(WebDriver driver, Variaveis dados) throws InterruptedException, IOException, UnsupportedFlavorException, FindFailed, AWTException {
        ...
        if (VerificarElemento.verificar(driver, "//*[contains(text(), 'Assistência a automóvel')]", "XPATH", 30)) {
          ...
          /*  if (VerificarElemento.verificar(driver, "//*[contains(@value, 'YOUSE_ID_2')]", "XPATH", 1)) {
            ...
            if (VerificarElemento.verificar(driver, "//*[contains(@data-track-name, 'Lavagem e higienização')]", "XPATH", 1)) {
                System.out.println("higienizacao");
                WebElement meuchk = driver.findElement(By.xpath("//*[contains(@data-track-name, 'Lavagem e higienização')]"));
                JavascriptExecutor js = (JavascriptExecutor) driver;
                js.executeScript("arguments[0].click();", meuchk);
            }

            if (VerificarElemento.verificar(driver, "//*[contains(@data-track-name, 'Serviço de leva e traz')]", "XPATH", 1)) {
                System.out.println("leva e traz");
                WebElement meuchk = driver.findElement(By.xpath("//*[contains(@data-track-name, 'Serviço de leva e traz')]"));
                JavascriptExecutor js = (JavascriptExecutor) driver;
                js.executeScript("arguments[0].click();", meuchk);
            }
            ...
            if (VerificarElemento.verificar(driver, "//*[contains(@data-track-name, 'Assistência a bike')]", "XPATH", 1)) {
                System.out.println("bike");
                WebElement meuchk = driver.findElement(By.xpath("//*[contains(@data-track-name, 'Assistência a bike')]"));
                JavascriptExecutor js = (JavascriptExecutor) driver;
                js.executeScript("arguments[0].click();", meuchk);
            }
