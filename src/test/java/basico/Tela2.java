...
public class Tela2 {
    ...
    public String preencherTela2(WebDriver driver, Variaveis dados) throws InterruptedException, AWTException, FindFailed, UnsupportedFlavorException, IOException {
        ...
        if (VerificarElemento.verificar(driver, "has_vehicle_false", "ID", 1)) {
            System.out.println("Achou a tela 2");

            if (VerificarElemento.verificar(driver, "radio-button__fake-radio", "CLASS", 1)) {
                System.out.println("Achou CHK");

                driver.findElement(By.className("radio-button__fake-radio")).click();
                ...
                if (VerificarElemento.verificar(driver, "//*[@id=\"container\"]/div[12]/div/a", "XPATH", 5)) {
                    System.out.println("JANELA PLACA ERRADA");
                ...
                if (posicao > -1) {
                    slctMarca.selectByVisibleText(dados.getMarca().toUpperCase());
                    Thread.sleep(5000);
                      ...
                      /*  if (dados.getVeiculo().contains("/")) {
                        ...
                        if (posicao > -1) {
                            ...
                            if (driver.findElements(By.xpath("//*[contains(text(), 'a gente ainda não oferece seguros para o mesmo modelo e ano do seu carro.')]")).size() > 0) {
                                ...
                                if (VerificarElemento.verificar(driver, "auto_order_flow_pricing_requirements_vehicle_attributes_usage", "ID", 30)) {
                                    WebElement cmbUtiliz = driver.findElement(By.id("auto_order_flow_pricing_requirements_vehicle_attributes_usage"));
                                    Select slcUtiliz = new Select(cmbUtiliz);
                                    if (dados.getComercial().toUpperCase().contains("PARTIC")) {
                                        slcUtiliz.selectByIndex(1);
                                    } else if (dados.getComercial().toUpperCase().contains("UBER")) {
                                        slcUtiliz.selectByIndex(2);
                                    }
                                ...
                                if (VerificarElemento.verificar(driver, "auto_order_flow_pricing_requirements_insured_person_attributes_address_attributes_zipcode", "ID", 30)) {
                                    for (int i = 0; i < dados.getCep1().length(); ) {
                                        char c = dados.getCep1().charAt(i);
                                        String s = new StringBuilder().append(c).toString();
                                        driver.findElement(By.id("auto_order_flow_pricing_requirements_insured_person_attributes_address_attributes_zipcode")).sendKeys(s);
                                        i++;
                                    }

                                    Thread.sleep(1000);
                                    driver.findElement(By.id("auto_order_flow_pricing_requirements_insured_person_attributes_address_attributes_number")).sendKeys("100");
                                }
    ...
    public boolean verificarAceita(WebDriver driver, Variaveis dados) throws IOException, UnsupportedFlavorException {
